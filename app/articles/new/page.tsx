...
export default function CreateArticle() {
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setUploadError("");
    setIsUploading(true);
    setUploadedImageUrl(""); // Reset url sebelumnya jika upload ulang
    ...
    try {
      ...
      // Menangani error jika NextJS mengembalikan format non-JSON (seperti halaman 413 Payload Too Large)
      ...
      if (!response.ok) {
        setUploadError(data.error || "Upload failed");
        setIsUploading(false);
        setPreviewUrl(""); // Hapus preview jika gagal upload
        return;
      }

      setUploadedImageUrl(data.url); // Set link gambar dari server
      setUploadError("");
      setIsUploading(false);
    } catch {
      setUploadError("Upload failed. Please try again.");
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // Perbaikan: Alert dibuat spesifik agar Anda tahu field mana yang error
    if (!uploadedImageUrl) {
      alert(
        "Mohon tunggu gambar selesai diupload atau pastikan upload berhasil!",
      );
      return;
    }
    if (!title) return alert("Judul artikel tidak boleh kosong!");
    if (!content) return alert("Isi artikel tidak boleh kosong!");
    if (!altText) return alert("Alt text gambar tidak boleh kosong!");

    // Perbaikan: Ikut mengirimkan altText ke Redux store
    dispatch(
      addArticle({ title, content, imageUrl: uploadedImageUrl, altText }),
    );
        ...
        <div className="bg-white rounded-2xl shadow-lg p-8 border border-slate-200">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            <div>
              ...
              {(previewUrl || uploadedImageUrl) && (
                <div className="mb-4 relative w-full h-64 bg-slate-100 rounded-lg overflow-hidden">
                  {/* Gunakan gambar biasa untuk preview untuk menghindari error Domain Config dari Next.js */}
                  <img
            ...
            <div className="flex justify-between items-center pt-6 border-t border-slate-200">
              ...
              <Button
                ...
                disabled={isUploading}
                /* Mencegah user klik Publish sebelum upload selesai */
              >
                <Plus size={18} className="mr-2" />
